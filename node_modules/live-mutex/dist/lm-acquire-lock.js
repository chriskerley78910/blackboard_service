...
if (index > 1) {
    try {
        ...
        client_1.log.error(`Could not parse your --json argument, try --json '{"port": 3091,"key": "foo"}' ... ` +
...
if (!Number.isInteger(v.port)) {
    client_1.log.error(chalk_1.default.magenta('Live-mutex: port could not be parsed to integer from command line input.'));
    client_1.log.error('Usage: lm_acquire_lock <key> <?port>');
    process.exit(1);
}
if (!v.key) {
    client_1.log.error(chalk_1.default.magenta('Live-mutex: no key passed at command line.'));
    client_1.log.error('Usage: lm_acquire_lock <key> <?port>');
    process.exit(1);
}
...
process.once('unhandledRejection', function (e) {
    client_1.log.error('unhandledRejection:', chalk_1.default.magenta(util.inspect(e)));
    process.exit(1);
});
process.once('uncaughtException', function (e) {
    client_1.log.error('uncaughtException:', chalk_1.default.magenta(util.inspect(e)));
    process.exit(1);
});
...
c.emitter.on('info', function () {
    client_1.log.debug(...arguments);
});
c.emitter.on('warning', function () {
    client_1.log.warn(...arguments);
});
c.emitter.on('error', function () {
    client_1.log.error(...arguments);
});
c.ensure().then(function (c) {
    ...
    c.lock(v.key, lockOptions, function (e) {
        if (e) {
            ...
            client_1.log.error(`To discover what is going on with the broker, use ${chalk_1.default.blueBright.bold('$ lm_inspect_broker -p <port> -h <host>')}.`);
            return process.exit(1);
        }
        client_1.log.debug(chalk_1.default.green.bold(`${chalk_1.default.italic('Acquired')} lock for key:`), `'${chalk_1.default.blueBright.bold(v.key)}'`);
        process.exit(0);
    });
});
